...
void SMonitor::initSCFrame() {
    ...
    pSCFrame->setReady([this](CCZwlrScreencopyFrameV1* r, uint32_t tv_sec_hi, uint32_t tv_sec_lo, uint32_t tv_nsec) {
        ...
        if (bytesPerPixel == 4)
            g_pCaptura->convertBuffer(pLS->screenBuffer);
        else if (bytesPerPixel == 3) {
            Debug::log(WARN, "24 bit formats are unsupported, captura may or may not work as intended!");
            data                          = g_pCaptura->convert24To32Buffer(pLS->screenBuffer);
