...
CLayerSurface::CLayerSurface(SMonitor* pMonitor) : m_pMonitor(pMonitor) {
    pSurface = makeShared<CCWlSurface>(g_pCaptura->m_pCompositor->sendCreateSurface());

    if (!pSurface) {
        Debug::log(CRIT, "The compositor did not allow captura a surface!");
        g_pCaptura->finish(1);
        return;
    }

    if (!g_pCaptura->m_bNoFractional) {
        pViewport = makeShared<CCWpViewport>(g_pCaptura->m_pViewporter->sendGetViewport(pSurface->resource()));
        ...
        pFractionalScale = makeShared<CCWpFractionalScaleV1>(g_pCaptura->m_pFractionalMgr->sendGetFractionalScale(pSurface->resource()));
        pFractionalScale->setPreferredScale([this](CCWpFractionalScaleV1* r, uint32_t scale120) { //
            ...
            wantsReload     = true;
            g_pCaptura->recheckACK();
        });
    }

    pLayerSurface = makeShared<CCZwlrLayerSurfaceV1>(
        g_pCaptura->m_pLayerShell->sendGetLayerSurface(pSurface->resource(), pMonitor->output->resource(), ZWLR_LAYER_SHELL_V1_LAYER_OVERLAY, "captura"));

    if (!pLayerSurface) {
        Debug::log(CRIT, "The compositor did not allow captura a layersurface!");
        g_pCaptura->finish(1);
        return;
    }
    ...
    wl_display_flush(g_pCaptura->m_pWLDisplay);
}
